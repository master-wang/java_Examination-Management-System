...
public class UserDaoimpl implements UserDao {
    ...
    public User loginAdmin(User loginUser) {
        try {
            ...
            User user = template.queryForObject(sql, new BeanPropertyRowMapper<User>(User.class), loginUser.getUsername(), loginUser.getPassword(),loginUser.getIs_admin());
            System.out.println("dao获取数据库得到的ｕｓｅｒ:" + user);
            return user;
        }catch (DataAccessException e){
            e.printStackTrace();
    ...
    public void addUserByAdmin(User user) {
        ...
        int update = template.update(sql, user.getUsername(), user.getPassword(),user.getIs_admin());
        System.out.println("添加成功："+update);
    }

    @Override
    public int updateUserInfos(User user) {
        String sql ="update user set name = ? , idCardNo = ?  ,phone = ?  where id = ?";
        int count = template.update(sql,user.getName(),user.getIdCardNo(),user.getPhone(),user.getId());
    ...
    public int updateUserbyId(User user) {
        ...
        int count = template.update(sql,user.getPassword(),user.getIs_admin(),user.getId());
        System.out.println("更新用户信息："+count);
        return count;
    }
